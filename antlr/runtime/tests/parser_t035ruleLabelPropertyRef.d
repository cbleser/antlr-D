module antlr.runtime.tests.parser_t035ruleLabelPropertyRef;
...
import antlr.runtime.tests.t035ruleLabelPropertyRefLexer;
import antlr.runtime.tests.t035ruleLabelPropertyRefParser;


template main_test(char_t) {
  void main_test(){
  t035ruleLabelPropertyRefLexer!(char_t) lexer;
  t035ruleLabelPropertyRefParser!(char_t) parser;

  { // Valid test 1
    auto input=new ANTLRStringStream!(char_t)("   a a a a   ");
    lexer=new t035ruleLabelPropertyRefLexer!(char_t)(input);
    auto stream=new CommonTokenStream!(char_t)(lexer);
    parser=new t035ruleLabelPropertyRefParser!(char_t)(stream);
    auto label=parser.a();
    // first token of rule b is the 2nd token (counting hidden tokens)
 
    assert(label.start.TokenIndex ==1);
    // first token of rule b is the 7th token (counting hidden tokens)
 
    assert(label.stop.TokenIndex == 7);
    assert(label.text == "a a a a");
  }  
