module antlr.runtime.tests.lexer_t011;
...
import antlr.runtime.tests.t011Lexer;

template main_test(char_t) {
  void main_test(){
	class t011Lexer_msg : t011Lexer!(char_t) {
	  ...
	  override void reportError(RecognitionExceptionT e) {
		...
		if (E !is null) {
		  assert(cast(char_t)E.getUnexpectedType == '-');
	...
	auto lexer=new t011Lexer_msg(input);
	//  while (true) {
