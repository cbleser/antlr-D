module antlr.runtime.tests.parser_t034tokenLabelPropertyRef;
...
import antlr.runtime.tests.t034tokenLabelPropertyRefLexer;
import antlr.runtime.tests.t034tokenLabelPropertyRefParser;

template main_test(char_t) {
  void main_test(){
  char_t[] name;

  t034tokenLabelPropertyRefLexer!(char_t) lexer;
  t034tokenLabelPropertyRefParser!(char_t) parser;

  { // Valid test 1
	auto input=new ANTLRStringStream!(char_t)("   a");
	lexer=new t034tokenLabelPropertyRefLexer!(char_t)(input);
	auto stream=new CommonTokenStream!(char_t)(lexer);
	parser=new t034tokenLabelPropertyRefParser!(char_t)(stream);
	parser.a();
  }  
